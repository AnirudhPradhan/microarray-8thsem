...
public class SolutionLevelReports {
    ...
    public static String getSLcompletefile(List<Experiment> exps) {
        String r = "";
        r = SolutionLevelReports.getHeader('b', ";") + "\n";
        for (Experiment exp : exps) {
            r = r + SolutionLevelReports.getSLreport(exp, ";", false, new DecimalFormat[0]) + "\n";
        }
        return r;
    }

    public static String getSLSinglefile(Experiment exp) {
        LinkedList<Experiment> aux = new LinkedList<Experiment>();
        aux.add(exp);
        String r = ExpLevelReports.getELreport(aux);
        r = r + "\n;\n";
        r = r + SolutionLevelReports.getSLreport(exp, ";", true, new DecimalFormat[0]);
        return r;
    }

    public static String getSLfile(Experiment exp) {
        return SolutionLevelReports.getSLreport(exp, ";", true, new DecimalFormat[0]);
    }

    public static String getSLreport(Experiment exp, String sep, boolean header, DecimalFormat ... decimalFormat) {
        String r = "";
        if (header) {
            r = r + SolutionLevelReports.getHeader(exp.getAnalysisType(), sep) + "\n";
        }
        for (Solution sol : exp.getSolutions()) {
            r = r + SolutionLevelReports.getSolutionLine(sol, exp.getAnalysisType(), sep, decimalFormat) + "\n";
        }
        return r;
    }
    ...
    protected static String getSolutionLine(Solution sol, char expType, String sep, DecimalFormat ... decimalFormat) {
        String r = "";
        r = SolutionLevelReports.lineL1(sol, expType, sep, decimalFormat);
        return r;
    }
    ...
    protected static String getHeader(char expType, String sep) {
        String r = "";
        r = SolutionLevelReports.headerL1(expType, sep);
        return r;
    }
    ...
    private static String headerL1(char expType, String sep) {
        ...
        r = expType == 'b' ? r + BIOQ + sep + TRIQN + sep + BIOQN + sep + GRQ + sep + PEQ + sep + SPQ : r + GRQ + sep + PEQ + sep + SPQ;
        return r;
    }
}
