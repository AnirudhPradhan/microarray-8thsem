...
public class PlaneOverlappingCrossover
implements Crossover {
    private static final Logger LOG = LoggerFactory.getLogger(PlaneOverlappingCrossover.class);
    private CrossoverStrategy grid = new GridStrategy();
    ...
    public List<AlgorithmIndividual> cross(int numberOfChildren, List<AlgorithmIndividual> selectedPopulation) {
        ...
        for (int i = 0; i < remainderCrossings[0]; ++i) {
            ...
            AlgorithmIndividual[] children = this.grid.cross(father, mother, TriGen.getInstance().getIndividualClassName());
            r.add(children[0]);
            r.add(children[1]);
        }
