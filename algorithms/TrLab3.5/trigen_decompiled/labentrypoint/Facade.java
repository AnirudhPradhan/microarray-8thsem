...
public class Facade {
    ...
    public static Experiment buildAnalysis(AnalysisResources resources, Options options, boolean triqFile) throws InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        Experiment r = AnalysisFactory.getExperiment(resources.getSolutions(), options, resources.getControl().getDataset(), options.getOutName());
        String auxFilePath = options.getOutFolder().getAbsolutePath() + SystemUtilities.getFileSep() + options.getOutName();
        String csv_ext = SystemUtilities.getSystemProperty("lab_extension");
        if (r != null) {
            r.computeAnalysis();
    ...
    public static void buildFiles(AnalysisResources resources, Options options, Experiment exp) throws InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        String auxFilePath = options.getOutFolder().getAbsolutePath() + SystemUtilities.getFileSep() + options.getOutName();
        String csv_ext = SystemUtilities.getSystemProperty("lab_extension");
        ...
        Facade.printFolder(excell_strings, options.getOutFolder().getAbsolutePath(), options.getOutName(), SystemUtilities.getSystemProperty("excel_folder"), SystemUtilities.getSystemProperty("excel_file"), csv_ext);
        String[] r_strings = FormatUtils.getRstrings(resources.getSolutions(), resources.getControl().getDataset().getDataset());
        Facade.printFolder(r_strings, options.getOutFolder().getAbsolutePath(), options.getOutName(), SystemUtilities.getSystemProperty("r_folder"), SystemUtilities.getSystemProperty("r_file"), csv_ext);
        if (resources.getControl().getDataset().getDatasetType() == 'b') {
            ...
            Facade.printFolder(genes_strings, options.getOutFolder().getAbsolutePath(), options.getOutName(), SystemUtilities.getSystemProperty("genes_folder"), SystemUtilities.getSystemProperty("genes_file"), gene_ext);
            if (exp != null) {
    ...
    public static void buildCompleteResultsFiles(AnalysisResources results, String pathToFolder) throws WrongOptionsException, InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        ...
        if (analysis != null) {
            ...
            Facade.printFolder(excell_strings, pathToFolder, control.getOutName(), SystemUtilities.getSystemProperty("excel_folder"), SystemUtilities.getSystemProperty("excel_file"), csv_ext);
            String[] r_strings = FormatUtils.getRstrings(solutions, dataset.getDataset());
            Facade.printFolder(r_strings, pathToFolder, control.getOutName(), SystemUtilities.getSystemProperty("r_folder"), SystemUtilities.getSystemProperty("r_file"), csv_ext);
            if (analysis.getAnalysisType() == 'b') {
                ...
                Facade.printFolder(genes_strings, pathToFolder, control.getOutName(), SystemUtilities.getSystemProperty("genes_folder"), SystemUtilities.getSystemProperty("genes_file"), gene_ext);
            }
        }
    }
    ...
    public static void buildEveryFiles(List<CommonAnalysisResources> resources, List<Experiment> experiments, String pathToFolder) throws InstantiationException, IllegalAccessException, ClassNotFoundException, WrongOptionsException, IOException, InterruptedException {
        for (CommonAnalysisResources r : resources) {
            ...
            Facade.buildAllResultFiles(r, pathToFolder + SystemUtilities.getFileSep() + r.getExperimentAlias());
        }
        for (Experiment e : experiments) {
            Facade.buildAllAnalysisFiles(e, pathToFolder + SystemUtilities.getFileSep() + e.getExperimentName());
        }
    }

    public static void buildAllAnalysisFiles(Experiment experiment, String pathToFolder) throws WrongOptionsException, InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        if (experiment != null) {
            String csv_ext = SystemUtilities.getSystemProperty("lab_extension");
            String triq_string = SolutionLevelReports.getSLfile(experiment);
            OutTextFile f1 = new OutTextFile(pathToFolder + SystemUtilities.getFileSep() + experiment.getExperimentName() + "_" + SystemUtilities.getSystemProperty("triq_file") + "." + csv_ext);
            f1.print(triq_string);
            f1.close();
            if (experiment.getAnalysisType() == 'b') {
                String sig_string = SignificanceReports.getSignificanceReportPA(experiment);
                OutTextFile f2 = new OutTextFile(pathToFolder + SystemUtilities.getFileSep() + experiment.getExperimentName() + "_" + SystemUtilities.getSystemProperty("significance_file") + "." + csv_ext);
                f2.print(sig_string);
                f2.close();
                String sig_stringn = SignificanceReports.getSignificanceReportP(experiment);
                OutTextFile f2n = new OutTextFile(pathToFolder + SystemUtilities.getFileSep() + experiment.getExperimentName() + "_" + SystemUtilities.getSystemProperty("significance_filen") + "." + csv_ext);
                f2n.print(sig_stringn);
                f2n.close();
            }
        }
    ...
    public static void buildAllResultFiles(CommonAnalysisResources resource, String pathToFolder) throws WrongOptionsException, InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        ...
        String gene_ext = SystemUtilities.getSystemProperty("gene_extension");
        String[] excell_strings = FormatUtils.getExcelStrings(solutions, dataset.getDataset());
        Facade.printFolder(excell_strings, pathToFolder, resource.getExperimentAlias(), SystemUtilities.getSystemProperty("excel_folder"), SystemUtilities.getSystemProperty("excel_file"), csv_ext);
        String[] r_strings = FormatUtils.getRstrings(solutions, dataset.getDataset());
        Facade.printFolder(r_strings, pathToFolder, resource.getExperimentAlias(), SystemUtilities.getSystemProperty("r_folder"), SystemUtilities.getSystemProperty("r_file"), csv_ext);
        if (resource.getAnalysisType() == 'b') {
            String[] genes_strings = GeneOntology.getGeneStrings(solutions, ((Biological)dataset).getGeneNames());
            Facade.printFolder(genes_strings, pathToFolder, resource.getExperimentAlias(), SystemUtilities.getSystemProperty("genes_folder"), SystemUtilities.getSystemProperty("genes_file"), gene_ext);
        }
    }
    ...
    public static void buildOPTfiles(Experiment experiment, Options options) throws WrongOptionsException, InstantiationException, IllegalAccessException, ClassNotFoundException, IOException, InterruptedException {
        String auxFilePath = options.getOutFolder().getAbsolutePath() + SystemUtilities.getFileSep() + options.getOutName();
        String csv_ext = SystemUtilities.getSystemProperty("lab_extension");
    ...
    private static void printFolder(String[] strings, String outFolderPath, String outName, String folderName, String fileName, String extension) throws FileNotFoundException {
