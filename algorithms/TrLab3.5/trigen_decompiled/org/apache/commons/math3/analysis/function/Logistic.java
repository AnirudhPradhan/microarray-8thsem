...
DifferentiableUnivariateFunction {
    ...
    public Logistic(double k, double m, double b2, double q, double a2, double n) throws NotStrictlyPositiveException {
        if (n <= 0.0) {
    ...
    private static double value(double mMinusX, double k, double b2, double q, double a2, double oneOverN) {
        return a2 + (k - a2) / FastMath.pow(1.0 + q * FastMath.exp(b2 * mMinusX), oneOverN);
    }
    ...
    implements ParametricUnivariateFunction {
        ...
        public double[] gradient(double x, double ... param) throws NullArgumentException, DimensionMismatchException, NotStrictlyPositiveException {
            ...
            double gk = Logistic.value(mMinusX, 1.0, b2, q, 0.0, oneOverN);
            double gm = factor2 * b2 * qExp;
            double gb = factor2 * mMinusX * qExp;
            double gq = factor2 * exp;
            double ga = Logistic.value(mMinusX, 0.0, b2, q, 1.0, oneOverN);
