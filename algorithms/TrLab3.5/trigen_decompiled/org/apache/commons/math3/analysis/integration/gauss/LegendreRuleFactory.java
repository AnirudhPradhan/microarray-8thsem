...
extends BaseRuleFactory<Double> {
    ...
    protected Pair<Double[], Double[]> computeRule(int numberOfPoints) throws DimensionMismatchException {
        ...
        for (int i = 0; i < iMax; ++i) {
            double a2 = i == 0 ? -1.0 : previousPoints[i - 1];
            double b2 = iMax == 1 ? 1.0 : previousPoints[i];
            double pma = 1.0;
            double pa = a2;
            double pmb = 1.0;
            double pb = b2;
            for (int j = 1; j < numberOfPoints; ++j) {
                int two_j_p_1 = 2 * j + 1;
                int j_p_1 = j + 1;
                double ppa = ((double)two_j_p_1 * a2 * pa - (double)j * pma) / (double)j_p_1;
                double ppb = ((double)two_j_p_1 * b2 * pb - (double)j * pmb) / (double)j_p_1;
                pma = pa;
                pa = ppa;
                pmb = pb;
                pb = ppb;
            ...
            while (!done) {
                ...
                pc = c2;
                for (int j = 1; j < numberOfPoints; ++j) {
                    double ppc = ((double)(2 * j + 1) * c2 * pc - (double)j * pmc) / (double)(j + 1);
                    pmc = pc;
                    pc = ppc;
                }
                if (done) continue;
            ...
            double d = (double)numberOfPoints * (pmc - c2 * pc);
            double w = 2.0 * (1.0 - c2 * c2) / (d * d);
            points[i] = c2;
            weights[i] = w;
