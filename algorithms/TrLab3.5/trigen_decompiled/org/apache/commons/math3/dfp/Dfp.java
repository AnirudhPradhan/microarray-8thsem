...
implements RealFieldElement<Dfp> {
    ...
    private static int compare(Dfp a2, Dfp b2) {
        ...
        for (int i = a2.mant.length - 1; i >= 0; --i) {
            if (a2.mant[i] > b2.mant[i]) {
                return a2.sign;
            }
            if (a2.mant[i] >= b2.mant[i]) continue;
            return -a2.sign;
        }
        return 0;
    }
        ...
        if (changed) {
            switch (rmode) {
                ...
                default: {
                    ...
                    Dfp a2 = this.subtract(result);
                    a2.sign = 1;
                    if (a2.greaterThan(half)) {
                        a2 = this.newInstance(this.getOne());
                        a2.sign = this.sign;
                        result = result.add(a2);
                    }
                    if (!a2.equals(half) || result.exp <= 0 || (result.mant[this.mant.length - result.exp] & 1) == 0) break;
                    a2 = this.newInstance(this.getOne());
                    a2.sign = this.sign;
                    result = result.add(a2);
                }
    ...
    public Dfp add(Dfp x) {
        ...
        Dfp a2 = this.newInstance(this);
        Dfp b2 = this.newInstance(x);
        Dfp result = this.newInstance(this.getZero());
        byte asign = a2.sign;
        byte bsign = b2.sign;
        a2.sign = 1;
    ...
    public Dfp add(double a2) {
        return this.add(this.newInstance(a2));
    }

    @Override
    public Dfp subtract(double a2) {
        return this.subtract(this.newInstance(a2));
    }

    @Override
    public Dfp multiply(double a2) {
        return this.multiply(this.newInstance(a2));
    }

    @Override
    public Dfp divide(double a2) {
        return this.divide(this.newInstance(a2));
    }

    @Override
    public Dfp remainder(double a2) {
        return this.remainder(this.newInstance(a2));
    }
