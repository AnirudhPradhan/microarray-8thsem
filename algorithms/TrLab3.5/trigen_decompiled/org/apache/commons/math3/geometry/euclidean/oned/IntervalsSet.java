    ...
    public BoundaryProjection<Euclidean1D> projectToBoundary(Point<Euclidean1D> point) {
        ...
        for (double[] a2 : this) {
            if (x < a2[0]) {
                ...
                double currentOffset = a2[0] - x;
                if (previousOffset < currentOffset) {
                    return new BoundaryProjection<Euclidean1D>(point, this.finiteOrNullPoint(previous), previousOffset);
                }
                return new BoundaryProjection<Euclidean1D>(point, this.finiteOrNullPoint(a2[0]), currentOffset);
            }
            if (x <= a2[1]) {
                double offset0 = a2[0] - x;
                double offset1 = x - a2[1];
                if (offset0 < offset1) {
                    return new BoundaryProjection<Euclidean1D>(point, this.finiteOrNullPoint(a2[1]), offset1);
                }
                return new BoundaryProjection<Euclidean1D>(point, this.finiteOrNullPoint(a2[0]), offset0);
            }
            previous = a2[1];
        }
