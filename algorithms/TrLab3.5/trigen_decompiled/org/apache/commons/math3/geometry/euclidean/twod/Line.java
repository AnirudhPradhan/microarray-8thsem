...
Embedding<Euclidean2D, Euclidean1D> {
    ...
    public void reset(Vector2D p1, Vector2D p2) {
        ...
        double d = FastMath.hypot(dx, dy = p2.getY() - p1.getY());
        if (d == 0.0) {
            this.angle = 0.0;
            this.cos = 1.0;
            this.sin = 0.0;
    ...
    public Line getReverse() {
        return new Line(this.angle < Math.PI ? this.angle + Math.PI : this.angle - Math.PI, -this.cos, -this.sin, -this.originOffset, this.tolerance);
    }
