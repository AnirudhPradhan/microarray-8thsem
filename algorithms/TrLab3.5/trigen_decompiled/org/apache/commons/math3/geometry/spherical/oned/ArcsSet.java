...
implements Iterable<double[]> {
                ...
                if (!wrapFirst) {
                    if (alpha < a2[0]) {
                        if (Double.isNaN(previous)) {
                            ...
                            return new BoundaryProjection<Sphere1D>(point, new S1Point(a2[0]), currentOffset);
                        }
                    }
                    if (alpha <= a2[1]) {
                        ...
                        if (offset0 < offset1) {
                            return new BoundaryProjection<Sphere1D>(point, new S1Point(a2[1]), offset1);
                        }
                        return new BoundaryProjection<Sphere1D>(point, new S1Point(a2[0]), offset0);
                    }
                }
            }
            previous = a2[1];
    ...
    public List<Arc> asList() {
        ...
        for (double[] a2 : this) {
            list.add(new Arc(a2[0], a2[1], this.getTolerance()));
        }
        return list;
    }
    ...
    public Side side(Arc arc) {
        ...
        for (double[] a2 : this) {
            ...
            if (syncedStart <= arcLength - this.getTolerance() || syncedEnd >= Math.PI * 2 + this.getTolerance()) {
                inMinus = true;
            }
            if (!(syncedEnd >= arcLength + this.getTolerance())) continue;
            inPlus = true;
        }
        if (inMinus) {
            if (inPlus) {
