...
implements BSPTreeVisitor<Sphere2D> {
    ...
    private void addContribution(SubCircle sub, boolean reversed, BSPTree<Sphere2D> node) {
        ...
        for (Arc a2 : arcs) {
            Vertex start = new Vertex((S2Point)circle.toSpace((Point)new S1Point(a2.getInf())));
            Vertex end = new Vertex((S2Point)circle.toSpace((Point)new S1Point(a2.getSup())));
            start.bindWith(circle);
            end.bindWith(circle);
            Edge edge = reversed ? new Edge(end, start, a2.getSize(), circle.getReverse()) : new Edge(start, end, a2.getSize(), circle);
