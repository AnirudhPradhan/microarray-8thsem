...
extends AbstractSubHyperplane<Sphere2D, Sphere1D> {
    ...
    public Side side(Hyperplane<Sphere2D> hyperplane) {
        ...
        return ((ArcsSet)this.getRemainingRegion()).side(thisCircle.getInsideArc(otherCircle));
    }
    ...
    public SubHyperplane.SplitSubHyperplane<Sphere2D> split(Hyperplane<Sphere2D> hyperplane) {
        ...
        if (angle < thisCircle.getTolerance()) {
            return new SubHyperplane.SplitSubHyperplane<Sphere2D>(null, this);
        }
        if (angle > Math.PI - thisCircle.getTolerance()) {
            return new SubHyperplane.SplitSubHyperplane<Sphere2D>(this, null);
        }
