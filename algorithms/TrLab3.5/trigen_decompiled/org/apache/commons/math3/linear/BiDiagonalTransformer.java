...
class BiDiagonalTransformer {
    ...
    private void transformToUpperBiDiagonal() {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            double xNormSqr = 0.0;
            for (int i = k; i < m; ++i) {
                double c2 = this.householderVectors[i][k];
                xNormSqr += c2 * c2;
            }
            double[] hK = this.householderVectors[k];
            this.main[k] = a2 = hK[k] > 0.0 ? -FastMath.sqrt(xNormSqr) : FastMath.sqrt(xNormSqr);
            if (a2 != 0.0) {
                int n2 = k;
                hK[n2] = hK[n2] - a2;
                for (j = k + 1; j < n; ++j) {
                    ...
                    alpha /= a2 * this.householderVectors[k][k];
                    for (i = k; i < m; ++i) {
                        hI = this.householderVectors[i];
                        int n3 = j;
            ...
            this.secondary[k] = b2 = hK[k + 1] > 0.0 ? -FastMath.sqrt(xNormSqr) : FastMath.sqrt(xNormSqr);
            if (b2 == 0.0) continue;
            int n4 = k + 1;
            hK[n4] = hK[n4] - b2;
            for (int i = k + 1; i < m; ++i) {
    ...
    private void transformToLowerBiDiagonal() {
        ...
        for (int k = 0; k < m; ++k) {
            ...
            xNormSqr = 0.0;
            for (int i = k + 1; i < m; ++i) {
                double c3 = this.householderVectors[i][k];
                xNormSqr += c3 * c3;
            }
            this.secondary[k] = b2 = hKp1[k] > 0.0 ? -FastMath.sqrt(xNormSqr) : FastMath.sqrt(xNormSqr);
            if (b2 == 0.0) continue;
            int n4 = k;
            hKp1[n4] = hKp1[n4] - b2;
            for (int j = k + 1; j < n; ++j) {
