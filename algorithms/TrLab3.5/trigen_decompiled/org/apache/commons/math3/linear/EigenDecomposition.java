...
public class EigenDecomposition {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            do {
                ...
                for (i3 = m - 1; i3 >= j; --i3) {
                    ...
                    t = (this.realEigenvalues[i3] - q) * s + 2.0 * c2 * h;
                    u = s * t;
                    this.realEigenvalues[i3 + 1] = q + u;
                    q = c2 * t - h;
                    for (int ia = 0; ia < n; ++ia) {
                        p = z[ia][i3 + 1];
                        z[ia][i3 + 1] = s * z[ia][i3] + c2 * p;
                        z[ia][i3] = c2 * z[ia][i3] - s * p;
                    }
                }
    ...
    private void findEigenVectorsFromSchur(SchurTransformer schur) throws MathArithmeticException {
        ...
        for (int idx = n - 1; idx >= 0; --idx) {
            ...
            for (i = idx - 2; i >= 0; --i) {
                ...
                l = i;
                if (Precision.equals(this.imagEigenvalues[i], 0.0)) {
                    Complex c2 = this.cdiv(-ra, -sa, w, q);
                    ...
                    Complex c3 = this.cdiv(x * r - z * ra + q * sa, x * s - z * sa - q * ra, vr, vi);
                    matrixT[i][idx - 1] = c3.getReal();
                    matrixT[i][idx] = c3.getImaginary();
    ...
    implements DecompositionSolver {
        ...
        public RealVector solve(RealVector b2) {
            ...
            if (b2.getDimension() != m) {
                throw new DimensionMismatchException(b2.getDimension(), m);
            }
            double[] bp = new double[m];
            for (int i = 0; i < m; ++i) {
                ArrayRealVector v = this.eigenvectors[i];
                double[] vData = v.getDataRef();
                double s = v.dotProduct(b2) / this.realEigenvalues[i];
                for (int j = 0; j < m; ++j) {
                    int n = j;
                    bp[n] = bp[n] + s * vData[j];
                }
