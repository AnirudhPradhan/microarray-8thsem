...
extends RealLinearOperator {
    ...
    public static JacobiPreconditioner create(RealLinearOperator a2) throws NonSquareOperatorException {
        int n = a2.getColumnDimension();
        if (a2.getRowDimension() != n) {
            throw new NonSquareOperatorException(a2.getRowDimension(), n);
        }
        double[] diag = new double[n];
        if (a2 instanceof AbstractRealMatrix) {
            AbstractRealMatrix m = (AbstractRealMatrix)a2;
            for (int i = 0; i < n; ++i) {
            ...
            for (int i = 0; i < n; ++i) {
                x.set(0.0);
                x.setEntry(i, 1.0);
                diag[i] = a2.operate(x).getEntry(i);
            }
        }
