...
public class MatrixUtils {
    ...
    public static void solveLowerTriangularSystem(RealMatrix rm, RealVector b2) throws DimensionMismatchException, MathArithmeticException, NonSquareMatrixException {
        ...
        for (int i = 0; i < rows; ++i) {
            ...
            double bi = b2.getEntry(i) / diag;
            b2.setEntry(i, bi);
            for (int j = i + 1; j < rows; ++j) {
                b2.setEntry(j, b2.getEntry(j) - bi * rm.getEntry(j, i));
            }
        }
    }

    public static void solveUpperTriangularSystem(RealMatrix rm, RealVector b2) throws DimensionMismatchException, MathArithmeticException, NonSquareMatrixException {
        if (rm == null || b2 == null || rm.getRowDimension() != b2.getDimension()) {
            throw new DimensionMismatchException(rm == null ? 0 : rm.getRowDimension(), b2 == null ? 0 : b2.getDimension());
        }
        if (rm.getColumnDimension() != rm.getRowDimension()) {
    ...
    public static RealMatrix blockInverse(RealMatrix m, int splitIndex) {
        ...
        RealMatrix a2 = m.getSubMatrix(0, splitIndex, 0, splitIndex);
        RealMatrix b2 = m.getSubMatrix(0, splitIndex, splitIndex1, n - 1);
        RealMatrix c2 = m.getSubMatrix(splitIndex1, n - 1, 0, splitIndex);
        ...
        RealMatrix tmp2 = d.subtract(c2.multiply(aInv).multiply(b2));
        ...
        RealMatrix result01 = aInv.multiply(b2).multiply(result11).scalarMultiply(-1.0);
        RealMatrix result10 = dInv.multiply(c2).multiply(result00).scalarMultiply(-1.0);
