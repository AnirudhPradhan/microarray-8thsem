...
implements UpdateAction {
    ...
    private final AtomicLong numberOfCalls = new AtomicLong(-1L);
    ...
    public void update(Network net, double[] features) {
        ...
        Gaussian neighbourhoodDecay = new Gaussian(currentLearning, 0.0, 1.0 / (double)currentNeighbourhood);
    ...
    private void updateNeighbouringNeuron(Neuron n, double[] features, double learningRate) {
        double[] update;
        double[] expect;
        while (!n.compareAndSetFeatures(expect = n.getFeatures(), update = this.computeFeatures(expect, features, learningRate))) {
        }
    }
    ...
    private Neuron findAndUpdateBestNeuron(Network net, double[] features, double learningRate) {
        ...
        double[] expect;
        Neuron best;
        while (!(best = MapUtils.findBest(features, net, this.distance)).compareAndSetFeatures(expect = best.getFeatures(), update = this.computeFeatures(expect, features, learningRate))) {
        }
        return best;
    }
    ...
    private double[] computeFeatures(double[] current, double[] sample, double learningRate) {
        ...
        return s.subtract(c2).mapMultiplyToSelf(learningRate).add(c2).toArray();
    }
}
