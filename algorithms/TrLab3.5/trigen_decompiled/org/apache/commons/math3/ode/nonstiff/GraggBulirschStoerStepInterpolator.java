    ...
    protected void computeInterpolatedStateAndDerivatives(double theta, double oneMinusThetaH) {
        ...
        for (int i = 0; i < dimension; ++i) {
            ...
            for (int j = this.currentDegree - 1; j > 3; --j) {
                ...
                cDot = d * (theta05 * cDot + c2);
                c2 = this.polynomials[j][i] + c2 * d * theta05;
            }
            int n = i;
            this.interpolatedState[n] = this.interpolatedState[n] + t4 * c2;
            int n2 = i;
            this.interpolatedDerivatives[n2] = this.interpolatedDerivatives[n2] + (t4 * cDot + t4Dot * c2) / this.h;
        }
        if (this.h == 0.0) {
