...
extends AbstractConvergenceChecker<PointValuePair> {
    ...
    public boolean converged(int iteration, PointValuePair previous, PointValuePair current) {
        ...
        double difference = FastMath.abs(p - (c2 = ((Double)current.getValue()).doubleValue()));
        return difference <= (size = FastMath.max(FastMath.abs(p), FastMath.abs(c2))) * this.getRelativeThreshold() || difference <= this.getAbsoluteThreshold();
    }
}
