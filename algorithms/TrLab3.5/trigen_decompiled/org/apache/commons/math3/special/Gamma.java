...
public class Gamma {
    ...
    public static double regularizedGammaP(double a2, double x) {
        return Gamma.regularizedGammaP(a2, x, 1.0E-14, Integer.MAX_VALUE);
    }

    public static double regularizedGammaP(double a2, double x, double epsilon, int maxIterations) {
        double ret;
        if (Double.isNaN(a2) || Double.isNaN(x) || a2 <= 0.0 || x < 0.0) {
            ...
            double an;
            double n = 0.0;
            for (sum = an = 1.0 / a2; FastMath.abs(an / sum) > epsilon && n < (double)maxIterations && sum < Double.POSITIVE_INFINITY; sum += (an *= x / (a2 + (n += 1.0)))) {
            }
            if (n >= (double)maxIterations) {
                throw new MaxCountExceededException(maxIterations);
            }
            ret = Double.isInfinite(sum) ? 1.0 : FastMath.exp(-x + a2 * FastMath.log(x) - Gamma.logGamma(a2)) * sum;
        }
        return ret;
    }

    public static double regularizedGammaQ(double a2, double x) {
        return Gamma.regularizedGammaQ(a2, x, 1.0E-14, Integer.MAX_VALUE);
    }

    public static double regularizedGammaQ(final double a2, double x, double epsilon, int maxIterations) {
        double ret;
        if (Double.isNaN(a2) || Double.isNaN(x) || a2 <= 0.0 || x < 0.0) {
            ret = Double.NaN;
        } else if (x == 0.0) {
            ...
            ContinuedFraction cf = new ContinuedFraction(){
                ...
                protected double getA(int n, double x) {
                    return 2.0 * (double)n + 1.0 - a2 + x;
                }

                protected double getB(int n, double x) {
                    return (double)n * (a2 - (double)n);
                }
            };
            ret = 1.0 / cf.evaluate(x, epsilon, maxIterations);
            ret = FastMath.exp(-x + a2 * FastMath.log(x) - Gamma.logGamma(a2)) * ret;
        }
        return ret;
    }
    ...
    public static double invGamma1pm1(double x) {
        ...
        if (t < 0.0) {
            double a2 = 6.116095104481416E-9 + t * 6.247308301164655E-9;
            double b2 = 1.9575583661463974E-10;
            b2 = -6.077618957228252E-8 + t * b2;
            b2 = 9.926418406727737E-7 + t * b2;
            b2 = -6.4304548177935305E-6 + t * b2;
            b2 = -8.514194324403149E-6 + t * b2;
            b2 = 4.939449793824468E-4 + t * b2;
            b2 = 0.026620534842894922 + t * b2;
            b2 = 0.203610414066807 + t * b2;
            b2 = 1.0 + t * b2;
            double c2 = -2.056338416977607E-7 + t * (a2 / b2);
            c2 = 1.133027231981696E-6 + t * c2;
            c2 = -1.2504934821426706E-6 + t * c2;
            c2 = -2.013485478078824E-5 + t * c2;
            c2 = 1.280502823881162E-4 + t * c2;
            c2 = -2.1524167411495098E-4 + t * c2;
            c2 = -0.0011651675918590652 + t * c2;
            c2 = 0.0072189432466631 + t * c2;
            c2 = -0.009621971527876973 + t * c2;
            c2 = -0.04219773455554433 + t * c2;
            c2 = 0.16653861138229148 + t * c2;
            c2 = -0.04200263503409524 + t * c2;
            c2 = -0.6558780715202539 + t * c2;
            c2 = -0.42278433509846713 + t * c2;
            ret = x > 0.5 ? t * c2 / x : x * (c2 + 0.5 + 0.5);
        } else {
            ...
            q = 1.0 + t * q;
            double c3 = -2.056338416977607E-7 + p / q * t;
            c3 = 1.133027231981696E-6 + t * c3;
            c3 = -1.2504934821426706E-6 + t * c3;
            c3 = -2.013485478078824E-5 + t * c3;
            c3 = 1.280502823881162E-4 + t * c3;
            c3 = -2.1524167411495098E-4 + t * c3;
            c3 = -0.0011651675918590652 + t * c3;
            c3 = 0.0072189432466631 + t * c3;
            c3 = -0.009621971527876973 + t * c3;
            c3 = -0.04219773455554433 + t * c3;
            c3 = 0.16653861138229148 + t * c3;
            c3 = -0.04200263503409524 + t * c3;
            c3 = -0.6558780715202539 + t * c3;
            c3 = 0.5772156649015329 + t * c3;
            ret = x > 0.5 ? t / x * (c3 - 0.5 - 0.5) : x * c3;
        }
        return ret;
    }
    ...
    public static double gamma(double x) {
        ...
        if (absX <= 20.0) {
            ...
            double gammaAbs = 2.5066282746310007 / x * FastMath.pow(y, absX + 0.5) * FastMath.exp(-y) * Gamma.lanczos(absX);
