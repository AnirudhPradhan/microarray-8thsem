...
implements RankingAlgorithm {
    ...
    private void resolveTie(double[] ranks, List<Integer> tiesTrace) {
        double c2 = ranks[tiesTrace.get(0)];
        int length = tiesTrace.size();
        switch (this.tiesStrategy) {
            case AVERAGE: {
                this.fill(ranks, tiesTrace, (2.0 * c2 + (double)length - 1.0) / 2.0);
                break;
            }
            case MAXIMUM: {
                this.fill(ranks, tiesTrace, c2 + (double)length - 1.0);
                break;
            }
            case MINIMUM: {
                this.fill(ranks, tiesTrace, c2);
                break;
            }
            case RANDOM: {
                Iterator<Integer> iterator = tiesTrace.iterator();
                long f = FastMath.round(c2);
