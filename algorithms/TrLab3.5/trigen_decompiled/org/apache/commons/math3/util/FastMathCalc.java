...
class FastMathCalc {
    ...
    private static void split(double d, double[] split) {
        if (d < 8.0E298 && d > -8.0E298) {
            double a2 = d * 1.073741824E9;
            split[0] = d + a2 - a2;
            split[1] = d - split[0];
        } else {
            double a3 = d * 9.313225746154785E-10;
            split[0] = (d + a3 - d) * 1.073741824E9;
            split[1] = d - split[0];
        }
    }
    ...
    private static void resplit(double[] a2) {
        double c2 = a2[0] + a2[1];
        double d = -(c2 - a2[0] - a2[1]);
        if (c2 < 8.0E298 && c2 > -8.0E298) {
            double z = c2 * 1.073741824E9;
            a2[0] = c2 + z - z;
            a2[1] = c2 - a2[0] + d;
        } else {
            double z = c2 * 9.313225746154785E-10;
            a2[0] = (c2 + z - c2) * 1.073741824E9;
            a2[1] = c2 - a2[0] + d;
        }
    }
    ...
    static void splitReciprocal(double[] in, double[] result) {
        double b2 = 2.384185791015625E-7;
        double a2 = 0.9999997615814209;
        if (in[0] == 0.0) {
            in[0] = in[1];
            in[1] = 0.0;
        }
    ...
    private static void quadMult(double[] a2, double[] b2, double[] result) {
        ...
        FastMathCalc.split(a2[0], xs);
        FastMathCalc.split(b2[0], ys);
        FastMathCalc.splitMult(xs, ys, zs);
        result[0] = zs[0];
        result[1] = zs[1];
        FastMathCalc.split(b2[1], ys);
        FastMathCalc.splitMult(xs, ys, zs);
    ...
    static double[] slowLog(double xi) {
        ...
        FastMathCalc.splitReciprocal(x, a2);
        x[0] = x[0] - 2.0;
        FastMathCalc.resplit(x);
        FastMathCalc.splitMult(x, a2, y);
        x[0] = y[0];
        x[1] = y[1];
        ...
        for (int i = LN_SPLIT_COEF.length - 2; i >= 0; --i) {
            FastMathCalc.splitMult(y, x2, a2);
            y[0] = a2[0];
            y[1] = a2[1];
            FastMathCalc.splitAdd(y, LN_SPLIT_COEF[i], a2);
            y[0] = a2[0];
            y[1] = a2[1];
        }
        FastMathCalc.splitMult(y, x, a2);
        y[0] = a2[0];
        y[1] = a2[1];
        return y;
    }
    ...
    static void printarray(PrintStream out, String name, int expectedLen, double[][] array2d) {
        out.println(name);
        FastMathCalc.checkLen(expectedLen, array2d.length);
        out.println("    { ");
        int i = 0;
    ...
    static void printarray(PrintStream out, String name, int expectedLen, double[] array) {
        out.println(name + "=");
        FastMathCalc.checkLen(expectedLen, array.length);
