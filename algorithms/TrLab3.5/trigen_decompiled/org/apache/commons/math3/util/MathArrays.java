...
public class MathArrays {
    ...
    public static void sortInPlace(double[] x, OrderDirection dir, double[] ... yList) throws NullArgumentException, DimensionMismatchException {
        ...
        ArrayList<Pair<Double, Integer>> list = new ArrayList<Pair<Double, Integer>>(len);
        for (int i = 0; i < len; ++i) {
            list.add(new Pair<Double, Integer>(x[i], i));
        }
        Comparator<Pair<Double, Integer>> comp = dir == OrderDirection.INCREASING ? new Comparator<Pair<Double, Integer>>(){
    ...
    public static double linearCombination(double[] a2, double[] b2) throws DimensionMismatchException {
        int len = a2.length;
        if (len != b2.length) {
            throw new DimensionMismatchException(len, b2.length);
        }
        if (len == 1) {
            return a2[0] * b2[0];
        }
