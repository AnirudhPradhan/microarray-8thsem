...
extends Chart<XYStyler, XYSeries> {
    ...
    public XYSeries addSeries(String seriesName, List<?> xData, List<? extends Number> yData, List<? extends Number> errorBars) {
        ...
        if (xData != null) {
            ...
            series = new XYSeries(seriesName, this.getGeneratedData(yData.size()), yData, errorBars);
        }
    ...
    public void paint(Graphics2D g) {
        for (XYSeries seriesXY : this.getSeriesMap().values()) {
            XYSeries.XYSeriesRenderStyle chartXYSeriesRenderStyle = seriesXY.getXYSeriesRenderStyle();
            if (chartXYSeriesRenderStyle != null) continue;
            seriesXY.setXYSeriesRenderStyle(((XYStyler)this.getStyler()).getDefaultSeriesRenderStyle());
        }
