                        ...
                        for c2 in self._cubeVec:
                            if c != c2 and not c2.deleted:
                                if c2.containPoint(itt, its, itg):
                                    ...
                                    break
                        total += 1
            if total > 0 and inc/total >= overlaped:
                c.deleted = True
...
class Cluster:
    ...
    def __init__(self, array3d:Array3D, cubes:Cubes):
        ...
        self._edgesMatrix = [list() for _ in range(self._width**2)]
        self._pRects = [Rects() for _ in range(array3d.T)]
        self._pCubes = cubes
    ...
    def EXPAND_T(self, cubes: Cubes, c:Cube, P:set, support:list, eDelta:list):
        ...
        while len(P) > 0:
            ...
            c1.copy(c)
            P.remove(x)
            if len(c1.T) == 0 or c1.T[-1] + 1 == x:  # TEMPORAL CONTIGUOUS CONSTRAINT
                c1.T.add(x)
                if len(c.T) == 0:
