"""
run_benchmark.py  (TCtriCluster)
---------------------------------
Run TCtriCluster on the Soares et al. 2024 benchmark datasets and evaluate
R, CE, RMS3 against ground-truth triclusters.

TCtriCluster adds a *temporal contiguity* constraint on top of triCluster:
biclusters across consecutive timepoints only are merged into triclusters.

Uses the same pre-formatted .tab files from Benchmark_Datasets/*/Tricluster/
that the triCluster runner uses.
"""
...
# ---------------------------------------------------------------------------
# Dataset registry — same 17 datasets as triCluster benchmark
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# TCtriCluster parameters (mirrors triCluster defaults)
# ---------------------------------------------------------------------------
TCTRICLUSTER_PARAMS = dict(
    winsz=0.03,
    min_times=2,
    min_samples=4,
    min_genes=8,
...
def run_dataset(name: str, tab_path, tsv_path: Path, gt_path: Path) -> dict:
    ...
    tc = TCtriCluster(**TCTRICLUSTER_PARAMS)
    t0 = time.time()
    raw = tc.fit(data)
    elapsed = time.time() - t0

    predicted = triclusters_to_records(raw)
